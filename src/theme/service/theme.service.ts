...
export class ThemeService implements CrudRepositoryContract<Theme> {
    ...
    async create(theme: Theme): Promise<Theme> {
        ...
        try {
            ...
            this.logger.error('Error creating a new theme', error.message);
    ...
    async findAll(): Promise<Theme[]> {
        ...
        try {
            ...
            this.logger.error('Error fetching all themes', error.stack);
        }
    ...
    async findById(id: number): Promise<Theme> {
        ...
        try {
            ...
            this.logger.error(`Error fetching theme with id: ${id}`, error.stack);
    ...
    async findByDescription(description: string): Promise<Theme[]> {
        ...
        try {
            ...
            this.logger.error(`Error searching themes with description: ${description}`, error.message);
            throw new InternalServerErrorException('Error searching themes by description.');
        }

        if (themes.length === 0) {
            this.logger.log(`No themes found with description: ${description}`);
            throw new NotFoundException(`No themes found with description: ${description}`);
        }
    ...
    async findByName(name: string): Promise<Theme[]> {
        ...
        try {
            ...
            this.logger.error(`Error searching themes with name: ${name}`, error.message);
    ...
    async update(theme: Theme): Promise<Theme> {
        this.logger.log(`Updating theme with id: ${theme.id}`);
            try {
                ...
                this.logger.error(`Error updating theme with id: ${theme.id}`, error.message);
    ...
    async delete(id: number): Promise<DeleteResult> {
        ...
        try {
            ...
            this.logger.error(`Error deleting theme with id: ${id}`, error.message);
